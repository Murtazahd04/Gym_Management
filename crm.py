...
def update_record():
	...
	c.execute("""UPDATE customers SET
		...
		WHERE oid = :oid""",
		{
			'name': n_entry.delete(0, END),
			'age': 	age_entry.delete(0, END),
			'address': address_entry.get(),
			'email': email_entry.get(),
			'join_date': join_date_entry.get(),
			'membership_plan': zipcode_entry.get(),
			'amount_due': zipcode_entry.get(),
			'membership_expiry': zipcode_entry.get(),
			'oid': id_entry.get(),
		})
	...
	membership_plan_entry.delete(0, END)
	amount_due_entry.delete(0, END)
	membership_expiry_entry.delete(0,END)
...
def add_record():
	...
	# Add New Record
	c.execute("INSERT INTO customers VALUES (:name, :age, :id, :address,:join_date, :email, :membership_plan, :amount_due ,:membership_expiry)",
		{
			'name':n_entry.delete(0, END), 
			'age': age_entry.delete(0, END),
			'id': 	id_entry.delete(0, END),
			'address': 	address_entry.delete(0, END),
			'join_date': join_date_entry.delete(0,END)
			'email': 	email_entry.delete(0, END),
			'membership_plan': 	membership_plan_entry.delete(0, END),
			'amount_due': 	amount_due.delete(0, END),
			'membership_expiry':membership_expiry_entry.delete(0,END),
		})
...
move_up_button = Button(button_frame, text="Renew Membership", command=up)
move_up_button.grid(row=0, column=5, padx=10, pady=10)

move_down_button = Button(button_frame, text="Clear Due Amount", command=down)
